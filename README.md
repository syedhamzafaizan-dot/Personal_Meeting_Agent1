🤖 AI Meeting Agent
🎯 New here? Check out README_SIMPLE.md for a friendlier introduction!
...
Stage 1: Transcript Intelligence Extraction - LLM extracts actions, decisions, risks with evidence
Stage 2: Owner Resolution Engine - Matches names/roles to people directory
Stage 3: Deadline Resolution Engine - Converts relative dates to ISO dates
Stage 4: Mini Autonomous Validation Agent - Validates owners, deadlines, consistency
Stage 5: Follow-up Message Generator - Creates personalized draft emails
Stage 6: Email Trigger Simulation - Logs trigger events without sending
...
meeting_output.json - Structured JSON with all extracted data
meeting_summary.txt - Human-readable summary
email_triggers.json - Simulated email trigger logs
...
sprint_planning.txt - Sprint planning meeting with multiple action items
product_sync.txt - Product sync with feature decisions
incident_review.txt - Post-incident review with technical tasks
🎯 Features
...
⚠️ Assumptions
Reference Date: Deadlines are resolved relative to the configured reference date
Name Matching: First names alone are matched if unique in the directory
OpenAI API: Uses GPT-4o-mini for cost efficiency (~$0.15 per 1M input tokens)
No Email Sending: Email triggers are logged but not sent
English Language: Optimized for English meeting transcripts
💡 Cost Optimization
